  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-[70%] h-full overflow-y-auto flex flex-col border-r border-slate-200 scrollbar-hide">
          
          {/* Video Player Area */}
          <div className="w-full aspect-video bg-black relative shadow-2xl">
            {currentLesson ? (
              (!currentLesson.is_preview && !isEnrolled) ? (
                <div className="w-full h-full bg-slate-50 flex flex-col items-center justify-center p-8 text-center space-y-6">
               ...
               <div className="w-full h-full flex items-center justify-center text-slate-400">Lesson not found</div>
            )}
          </div>
          ...
          <div className="p-8 lg:p-12 max-w-4xl mx-auto w-full space-y-10">
            ...
            <div className="space-y-4 border-b border-slate-200/60 pb-8">
              <div className="flex items-center gap-3">
                ...
                {isCompleted && (
                  <span className="px-3 py-1 bg-sky-500/10 text-sky-400 border border-sky-500/20 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1">
            ...
            <div className="bg-white/80 backdrop-blur-sm border border-slate-200 rounded-3xl p-8 shadow-xl">
              <h2 className="text-2xl font-semibold mb-6 flex items-center gap-3">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-blue-600">
                  {currentLesson?.has_assignment ? (
                    <path strokeLinecap="round" strokeLinejoin="round" d="M10.125 2.25h-4.5c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125v-9M10.125 2.25h.375a9 9 0 019 9v.375M10.125 2.25A3.375 3.375 0 0113.5 5.625v1.5c0 .621.504 1.125 1.125 1.125h1.5a3.375 3.375 0 013.375 3.375M9 15l2.25 2.25L15 12" />
                  ) : (
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  )}
                </svg>
                {currentLesson?.has_assignment ? "Lesson Assignment" : "Lesson Completion"}
              </h2>
              
              <div className="space-y-6">
                {currentLesson?.has_assignment && (
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-600">Submission Link (GitHub / Google Drive)</label>
                    <textarea 
                      value={assignmentUrl}
                      onChange={(e) => setAssignmentUrl(e.target.value)}
                      placeholder="https://github.com/yourusername/project"
                      className="w-full bg-slate-50 border border-slate-300 hover:border-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl p-4 text-slate-900 placeholder:text-slate-400 transition-all min-h-[100px] resize-y"
                    />
                  </div>
                )}
                ...
                <div className="flex items-center justify-between pt-4">
                  <p className="text-xs text-slate-400 max-w-[250px]">
                    {currentLesson?.has_assignment 
                      ? "By marking this as complete, you verify that you have finished watching the lesson and submitted your required work."
                      : "By clicking complete, you verify that you have finished watching this lesson in its entirety."}
                  </p>
                  
                  <button 
                    onClick={handleCompleteLesson}
                    disabled={submitting || (currentLesson?.has_assignment ? !assignmentUrl : false)} 
                  ...
                  >
                    {submitting ? (
                      ...
                      currentLesson?.has_assignment ? "Update Submission" : "Lesson Completed"
                    ) : (
                      "Complete Lesson"
                    )}
                  </button>
        ...
        <div className="w-full lg:w-[30%] h-full bg-white/30 overflow-y-auto flex flex-col">
          <div className="p-6 border-b border-slate-200/60 sticky top-0 bg-white/90 backdrop-blur-xl z-10">
            <h2 className="text-xl font-bold text-slate-900">Course Syllabus</h2>
            <p className="text-sm text-slate-600 mt-1">{allLessons.length} Modules</p>
          </div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-start gap-4 p-4 rounded-xl text-left transition-all duration-200 group ${
                    isActive 
                      ? "bg-blue-500/10 border border-blue-500/50 shadow-inner" 
                      : "hover:bg-slate-100 border border-transparent hover:border-slate-300/50"
                  }`}
                >
                  <div className={`mt-1 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
                    isActive ? "bg-blue-600 text-white shadow-lg shadow-blue-500/30" : "bg-slate-200 text-slate-600 group-hover:bg-slate-700"
                  }`}>
                    {index + 1}
                  </div>
                  <div className="flex-1">
                    <h4 className={`font-medium mb-1 line-clamp-2 ${isActive ? "text-blue-100" : "text-slate-700 group-hover:text-slate-900"}`}>
                      {lesson.title}
                    </h4>
                    {lesson.is_preview && !isActive && (
                      <span className="text-[10px] text-sky-400 uppercase tracking-wider font-semibold">Free Preview</span>
                    )}
                  </div>
                  
                  {/* If we loaded all progress, we could show a checkmark here. For now, just show active indicator */}
                  {isActive && (
                     <div className="w-2 h-2 rounded-full bg-blue-400 self-center shadow-[0_0_8px_rgba(129,140,248,0.8)]"></div>
                  )}
                </button>
              );
            })}
